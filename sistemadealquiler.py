...
class SillaApp:
    def __init__(self, root):
        ...
        self.inventario_informales_inicial = 800
        self.inventario_formales_inicial = 400
    ...
    def seleccionar_paquete(self):
        ...
        self.inventario_label.config(text=f"Sillas informales disponibles: {self.inventario_informales}\nSillas formales disponibles: {self.inventario_formales}")
    ...
    def devolver_sillas(self):
        ...
        if tipo_silla == "Informal":
            if cantidad > (self.inventario_informales_inicial - self.inventario_informales):
                self.resultado_label.config(text=f"No puede devolver más sillas informales de las que tenía inicialmente.")
                return
            self.inventario_informales += cantidad
        else:
            if cantidad > (self.inventario_formales_inicial - self.inventario_formales):
                self.resultado_label.config(text=f"No puede devolver más sillas formales de las que tenía inicialmente.")
                return
            self.inventario_formales += cantidad
    ...
    def create_widgets(self):
        ...
        self.inventario_label = tk.Label(frame, text=f"Sillas informales disponibles: {self.inventario_informales}\nSillas formales disponibles: {self.inventario_formales}", font=("Helvetica", 12))
        self.inventario_label.grid(row=13, column=1, columnspan=2)

        tk.Button(frame, text="Salir", command=self.root.quit).grid(row=14, column=1, columnspan=2)
